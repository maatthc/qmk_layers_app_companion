...
def parser():
    ...
    parser.add_argument(
        "-i",
        "--client_ip",
        help="IP address of the remote host - if not provided, local network discovery will be used.",
    )
    parser.add_argument(
        "-p",
        "--client_port",
        type=int,
        default=1977,
        help="Port to be used by the client - Default : 1977",
    )
    parser.add_argument(
        "-c",
        "--client",
        action="store_true",
        help="Receives keystrokes from remote host to display Keyboard layout",
    )
